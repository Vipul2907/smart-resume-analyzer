...
<body><main>
  ...
  <section><h2>Skills</h2>@forelse($topSkills as $skill)<p><strong>{{ $skill['name'] }}</strong> — {{ $skill['proficiency'] }}%{{ $skill['target'] ? ' / '.$skill['target'].'% target' : '' }}</p><div class="bar"><div style="width: {{ min(100, $skill['proficiency']) }}%"></div></div>@empty<p class="muted">No skills have been measured yet.</p>@endforelse</section>
  <section><h2>Career goals</h2>@forelse($goals as $goal)<p><strong>{{ $goal->title }}</strong> — {{ $goal->progress ?? 0 }}% complete</p><div class="bar"><div style="width: {{ min(100, (int) ($goal->progress ?? 0)) }}%"></div></div>@empty<p class="muted">No career goals have been created yet.</p>@endforelse</section>
